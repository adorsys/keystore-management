...
import de.adorsys.keymanagement.core.view.entity.KeyPair;
...
public class PairView extends UpdatingView<KeyPair> {
    ...
    private static final SQLParser<KeyPair> PARSER = SQLParser.forPojoWithAttributes(
            KeyPair.class,
            createAttributes(KeyPair.class, GETTER_METHODS_ONLY, PROP_NAME)
    );
    ...
    public PairView(KeySource source, Collection<Index<KeyPair>> indexes) {
        ...
        keys.addAll(
                source.aliasesFor(ProvidedKeyPair.class)
                        .map(it -> new KeyPair(it, null, source.asPair(it)))
    ...
    protected KeyPair viewFromId(String ofKey) {
        return new KeyPair(ofKey, null, source.asPair(ofKey)); // FIXME fill metadata
    }
